...
public:
	...
	DoubleLinkList<DataType>::iterator erase(int index) {
	...
	DoubleLinkList<DataType>::iterator erase(Node* node) {

		if (node == nullptr || findIndex(node) == -1) return iterator(nullptr);
	...
	DoubleLinkList<DataType>::iterator erase(DoubleLinkList<DataType>::iterator iter) {

		if (iter == end()) return end();

		Node* pre = iter._currentNode->previous;
		Node* cur = const_cast<Node*>(iter._currentNode);
		Node* temp = iter._currentNode->next;
